...
export default function MobileAppBanner() {
  ...
  return (
    <>
      ...
      `}} />
      <div 
        className="fixed top-0 left-0 right-0 z-[100] h-[64px] bg-[#0A0A0A] text-white px-4 flex items-center justify-between shadow-lg md:hidden border-b-[2px] border-transparent"
        style={{ borderImage: "linear-gradient(to right, #3b82f6, #ec4899) 1" }}
      >
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[10px] font-medium text-gray-400">{t("banner.app_subtitle")}</span>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Link href="/download" onClick={() => setIsVisible(false)} className="bg-white text-black text-[11px] font-black uppercase px-4 py-2 rounded-full shadow-sm active:scale-95 transition-transform tracking-wider">
            {t("banner.open_btn")}
          </Link>
          <button onClick={handleClose} className="text-gray-400 hover:text-white p-2 -mr-2 transition-colors">
