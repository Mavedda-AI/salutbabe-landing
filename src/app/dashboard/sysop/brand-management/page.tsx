    ...
    <div className="min-h-screen bg-[#F8F9FA] font-sans">
      ...
      <div className="max-w-[1400px] mx-auto px-4 py-6 space-y-6 pb-20">
        ...
        <div className={`${cardClass} p-2 flex gap-1`}>
          {([['all', 'Tümü'], ['pending', <div className="flex items-center gap-1.5"><HugeiconsIcon icon={Timer02Icon} size={16} /> Bekleyen</div>], ['blocked', <div className="flex items-center gap-1.5"><HugeiconsIcon icon={Cancel01Icon} size={16} /> Engelli</div>]] ).map(([id, label]: any) => (
            ...
            {modal.type === 'detail' && modal.brand && (
              <>
                ...
                <div className="p-6 space-y-3">
                  {[['Durum', modal.brand.status], ['Kategori', modal.brand.category], ['Ürün Sayısı', String(modal.brand.products)], ['Satıcı Sayısı', String(modal.brand.sellers)], ['GMV', modal.brand.gmv], ['Kayıt Tarihi', modal.brand.registeredAt]].map(([l, v]: any, i: number) => (
