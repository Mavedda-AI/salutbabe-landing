...
export function ModerationView() {
  ...
  const [viewMode, setViewMode] = useState<'list' | 'grid2' | 'grid4' | 'swipe'>('swipe');
    ...
    <div className={`mx-auto pb-24 animate-fade-in ${viewMode === 'grid4' ? 'max-w-[900px]' : 'max-w-[600px]'}`}>
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center bg-white rounded-xl border border-gray-200 p-0.5 ml-1">
              <button onClick={() => setViewMode('swipe')} className={`w-8 h-8 flex items-center justify-center rounded-lg transition-all ${viewMode === 'swipe' ? 'bg-[#111827] text-white' : 'text-gray-400 hover:text-gray-700'}`} title="Tinder">
      ...
      {currentList.length === 0 ? (
        ...
        <div className="flex flex-col items-center justify-center py-10">
          <div className="relative w-full max-w-[360px] aspect-[4/5] bg-white rounded-[32px] overflow-hidden shadow-[0_20px_40px_rgba(0,0,0,0.1)] border border-gray-100 flex flex-col group">
             <img src={currentList[0].image} alt={currentList[0].name} className="w-full flex-1 object-cover" />
             <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent pointer-events-none"></div>
             <div className="absolute bottom-0 left-0 right-0 p-6 pt-12 flex flex-col pointer-events-none">
               <h2 className="text-[24px] font-black text-white leading-tight mb-1">{currentList[0].name}</h2>
               <p className="text-[13px] font-bold text-gray-300">{currentList[0].seller} • {currentList[0].date}</p>
               <div className="flex items-center gap-2 mt-3">
                 <span className="bg-white/20 backdrop-blur-md px-3 py-1 rounded-full text-white font-black text-[14px] shadow-sm">{currentList[0].price}</span>
                 <span className="bg-purple-500/80 backdrop-blur-md px-3 py-1 rounded-full text-white font-black text-[12px] shadow-sm">{currentList[0].category}</span>
               </div>
             </div>
          </div>
          ...
          <div className="flex items-center gap-6 mt-8">
            {tab === 'pending' ? (
              <>
                <button onClick={() => { setDetail(currentList[0]); }} className="w-16 h-16 rounded-full bg-white shadow-xl flex items-center justify-center text-blue-500 hover:scale-110 active:scale-95 transition-transform border border-gray-100"><span className="text-[24px]">💬</span></button>
                <button onClick={() => handleApprove(currentList[0])} className="w-20 h-20 rounded-full bg-white shadow-xl flex items-center justify-center text-green-500 hover:scale-110 active:scale-95 transition-transform border border-gray-100"><span className="text-[40px]">💚</span></button>
              </>
            ) : (
              <button onClick={() => handleUndo(currentList[0])} className="w-20 h-20 rounded-full bg-white shadow-xl flex items-center justify-center text-orange-500 hover:scale-110 active:scale-95 transition-transform border border-gray-100"><span className="text-[32px]">↩</span></button>
            )}
          </div>
        </div>
      ) : (
        <div className={`mt-3 ${viewMode === 'list' ? 'space-y-3' : viewMode === 'grid2' ? 'grid grid-cols-2 gap-3' : 'grid grid-cols-4 gap-3'}`}>
          {currentList.map(product => {
            ...
            {/* LIST VIEW */}
            return (
              <div key={product.id} className="bg-white rounded-[20px] border border-gray-100 shadow-sm overflow-hidden mb-3">
