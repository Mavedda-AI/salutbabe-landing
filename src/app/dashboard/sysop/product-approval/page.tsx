...
export default function ProductApprovalPage() {
  ...
  const renderCardInner = (listing: any, index: number) => {
    const isCol = layout === '4';
    return (
      <div className={`relative flex w-full h-full ${isCol ? 'flex-col' : 'flex-col sm:flex-row'} overflow-hidden rounded-3xl border border-gray-200 dark:border-white/10 bg-white dark:bg-[#12141C] shadow-2xl`}>
        ...
        {/* Image Section */}
        <div className={`relative shrink-0 p-3 bg-gray-50 dark:bg-black/20 ${isCol ? 'w-full h-48 sm:h-56' : 'h-64 sm:h-auto sm:w-64'}`}>
  ...
  return (
    <div className="space-y-6">
      {loading ? (
        ...
        <>
          {/* Header & Controls */}
          <div className="flex flex-col sm:flex-row sm:items-center justify-between bg-white dark:bg-[#12141C] p-5 rounded-3xl border border-gray-200 dark:border-white/10 shadow-sm gap-4">
            <div>
              <h2 className="text-xl font-black text-gray-900 dark:text-white">Onay Bekleyen İlanlar</h2>
              <p className="text-[13px] font-medium text-gray-500 mt-1">
                Şu anda onayınızı bekleyen toplam <span className="font-black text-gray-900 dark:text-white">{listings.length}</span> ilan bulunuyor.
              </p>
            </div>
            <div className="flex items-center gap-1.5 bg-gray-100 dark:bg-white/5 p-1.5 rounded-2xl shrink-0">
              <button 
                onClick={() => setLayout('1')} 
                className={`w-12 h-10 rounded-xl font-black text-[14px] transition-all flex items-center justify-center ${layout === '1' ? 'bg-white dark:bg-white/10 shadow-sm text-gray-900 dark:text-white' : 'text-gray-500 hover:text-gray-900 dark:hover:text-white'}`}
              >
                1
              </button>
              <button 
                onClick={() => setLayout('2')} 
                className={`w-12 h-10 rounded-xl font-black text-[14px] transition-all flex items-center justify-center ${layout === '2' ? 'bg-white dark:bg-white/10 shadow-sm text-gray-900 dark:text-white' : 'text-gray-500 hover:text-gray-900 dark:hover:text-white'}`}
              >
                2
              </button>
              <button 
                onClick={() => setLayout('4')} 
                className={`w-12 h-10 rounded-xl font-black text-[14px] transition-all flex items-center justify-center ${layout === '4' ? 'bg-white dark:bg-white/10 shadow-sm text-gray-900 dark:text-white' : 'text-gray-500 hover:text-gray-900 dark:hover:text-white'}`}
              >
                4
              </button>
            </div>
            ...
            <div className="relative w-full max-w-4xl mx-auto h-[650px] sm:h-[500px] flex items-center justify-center perspective-[2000px] mt-8 mb-12">
              {listings.slice(0, 3).reverse().map((listing) => {
                ...
                return (
                  ...
                  >
                    {renderCardInner(listing, actualIndex)}
                  </div>
                );
              })}
            </div>
          ) : (
            <div className={`grid gap-6 ${layout === '2' ? 'grid-cols-1 lg:grid-cols-2' : 'grid-cols-1 md:grid-cols-2 xl:grid-cols-4'}`}>
