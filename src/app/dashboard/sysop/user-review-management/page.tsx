        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={() => setModal(null)}>
          <div onClick={e => e.stopPropagation()} className="w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden">
            ...
            <div className="p-6 space-y-3">
              {[['Yazan', modal.reviewer], ['Hedef', modal.target], ['Tür', modal.type], ['Puan', <div className="flex gap-0.5">{Array.from({length: modal.rating}).map((_, i) => <HugeiconsIcon key={'s-'+i} icon={StarIcon} size={14} className="text-yellow-400" />)}{Array.from({length: 5 - modal.rating}).map((_, i) => <HugeiconsIcon key={'e-'+i} icon={StarIcon} size={14} className="text-gray-300" />)}</div>], ['Tarih', modal.date]].map(([l, v]: any, i: number) => (
