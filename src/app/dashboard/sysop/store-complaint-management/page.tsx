  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-[1400px] mx-auto pb-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
         <div>
           <h1 className={`text-2xl font-black tracking-tight ${isDark ? 'text-white' : 'text-[#1A1D1F]'}`}>Mağaza Şikayet Yönetimi</h1>
           <p className={`text-[13px] font-medium mt-1 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Satıcı/Mağaza profillerine gelen ihlalleri kontrol edin.</p>
         </div>
      </div>

      <div className={`${cardClass} p-8 text-center text-gray-500`}>
         {loading ? 'Yükleniyor...' : 'Mağaza şikayetleri tablosu burada yer alacaktır.'}
      </div>
    </div>
  );
}
