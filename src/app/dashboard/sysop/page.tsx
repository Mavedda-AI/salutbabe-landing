                ...
                <div className="flex items-center gap-2 relative">
                  <div className="relative">
                    ...
                    {activeDropdown === 'visitor' && (
                      <div className={`absolute top-full mt-1 right-0 w-24 rounded-lg shadow-xl z-50 overflow-hidden border ${isDark ? 'bg-[#1A1D1F] border-white/10' : 'bg-white border-gray-100'}`}>
                        {['Günlük', 'Haftalık', 'Aylık'].map(opt => (
                          <div key={opt} onClick={(e) => { e.stopPropagation(); alert(`${opt} ziyaretçi seçildi.`); setActiveDropdown(null); }} className={`px-3 py-2 text-[10px] font-bold cursor-pointer transition-colors ${isDark ? 'hover:bg-white/10 text-gray-300' : 'hover:bg-gray-50 text-[#374151]'}`}>{opt}</div>
                        ))}
                      </div>
                    )}
                  </div>
