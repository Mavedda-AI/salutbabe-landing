...
for line in input_data.strip().split('\n'):
    ...
    if line.startswith('## '):
        ...
        name = match.group(1) if match else line.replace('## ', '')
        product_count = random.randint(1000, 6000)
        output += f"    {{\n      id: 'CAT-{cat_id}', name: '{name}', productCount: {product_count}, status: 'Aktif',\n      subcategories: [\n"
        cat_id += 1
        sub_id = 1
        current_cat = name
        ...
        sub_id_str = str(sub_id).zfill(2)
        output += f"        {{ id: 'SUB-{cat_id-1}{sub_id_str}', name: '{name}', productCount: {product_count}, status: 'Aktif' }},\n"
        sub_id += 1

if current_cat:
    output += '      ]\n    }\n  ];\n'

with open('categories_output.txt', 'w', encoding='utf-8') as f:
    f.write(output)
